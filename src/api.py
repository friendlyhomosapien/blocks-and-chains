...
def addTransaction():
    ...
    if all(k in jsonData for k in (['sender', 'receiver', 'amount', 'currency'])):
...
def getChain():
    ...
    return jsonify({ 'chain': BlockchainInstance.getChain() }), 200


@bp.route('/chain/validate', methods=['POST'])
def chainValidate():
    if BlockchainInstance.validateChain():
        return jsonify({ 'success': True, 'msg': 'chain is valid' }), 200

    return jsonify({ 'success': True, 'msg': 'chain is invalid' }), 500
